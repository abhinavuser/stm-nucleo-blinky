...
CFLAGS += $(ARCH_FLAGS) $(GENLINK_CPPFLAGS) -I$(OPENCM3_DIR)/include
...
check-lib:
	@test -f "$(OPENCM3_DIR)/lib/libopencm3_stm32f3.a" || ( \
		echo "libopencm3 for STM32F3 is missing."; \
		echo "Run: powershell -ExecutionPolicy Bypass -File scripts/setup_libopencm3.ps1"; \
		exit 1)
